...
/* void Nand_Read_Test(){
	...
	for(i=0; i<10; i++){
		...
		//输出ascii字符
		for (j=0; j<16; j++){
			if (*(s + 16*i + j) < 0x20 || *(s + 16*i + j) > 0x7e)
				SendChar('.');
			else
				SendChar(*(s + 16*i + j));
		}
...
//Nand -> SDRAM 复制函数
void Nand_Copy_to_SDRAM(unsigned int source_addr, unsigned short * dst_addr, int len){
	int buf_len = 64;
	...
	while(1){
		Nand_Read(source_addr, buf, buf_len);
		
		for(i=0; i<buf_len;){
			*dst_addr = buf[i+1] << 8 | buf[i];
			dst_addr++;
			i += 2;
		}
		source_addr += buf_len;
